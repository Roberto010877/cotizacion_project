...
class OrdenCompra(BaseModel):
    ...
    proveedor = models.ForeignKey(
        Proveedor, on_delete=models.CASCADE, related_name='ordenes_compra')
    fecha_entrega_prevista = models.DateField(
        blank=True, null=True, verbose_name="Fecha de Entrega Prevista")
...
class DetalleOrdenCompra(BaseModel):
    ...
    producto_servicio = models.ForeignKey(
        ProductoServicio, on_delete=models.PROTECT)
    ...
    def __str__(self):
        return f"{self.cantidad} x {self.producto_servicio.nombre} en Orden de Compra {self.orden_compra.id}"
